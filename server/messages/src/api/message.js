...
module.exports = (app, channel, io) => {
  ...
  io.on("connection", (socket) => {
    socket.on("registerUser", async (userId) => {
      ...
      try {
        const unreadMessages = await service.getUnreadMessages(userId);
        io.to(userId).emit("unreadMessages", unreadMessages);
    ...
    socket.on("readMessage", async (data) => {
      console.log("READMESAAGE",data.sender,data.receiver)
      // Clear unread notifications for the user
      try {
        await service.readMessage(data);
        io.to(data.sender).emit("messageSeened",data.receiver);
        const unreadMessages = await service.getUnreadMessages(data.receiver);
        io.to(data.receiver).emit("unreadMessages", unreadMessages);
      } catch (error) {
        console.error(error)
      }
    });
